...
function textRun(value, size, color, bold = false) {
  return `<a:r><a:rPr lang="zh-CN" sz="${size}"${bold ? ' b="1"' : ''}><a:solidFill><a:srgbClr val="${color}"/></a:solidFill><a:latin typeface="Microsoft YaHei"/><a:ea typeface="Microsoft YaHei"/></a:rPr><a:t xml:space="preserve">${xml(value)}</a:t></a:r>`
}
...
function imageShape(image, id) {
  const box = {
    x: Math.round(image.x * EMU_PER_INCH),
    y: Math.round(image.y * EMU_PER_INCH),
    width: Math.round(image.w * EMU_PER_INCH),
    height: Math.round(image.h * EMU_PER_INCH),
  }
  let x = box.x
  let y = box.y
  let width = box.width
  ...
  if (image.fit === 'contain') {
    if (imageRatio > boxRatio) {
      height = Math.round(box.width / imageRatio)
      y += Math.round((box.height - height) / 2)
    } else {
      width = Math.round(box.height * imageRatio)
      x += Math.round((box.width - width) / 2)
    }
  } else if (imageRatio > boxRatio) {
    const visible = boxRatio / imageRatio
    const horizontal = Math.round((1 - visible) * 50000)
    crop = `<a:srcRect l="${horizontal}" r="${horizontal}"/>`
  } else {
    const visible = imageRatio / boxRatio
    const vertical = Math.round((1 - visible) * 50000)
    crop = `<a:srcRect t="${vertical}" b="${vertical}"/>`
  }
  return `<p:pic><p:nvPicPr><p:cNvPr id="${id}" name="Embedded image ${id}"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="${image.relationshipId}"/>${crop}<a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="${x}" y="${y}"/><a:ext cx="${width}" cy="${height}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:ln><a:noFill/></a:ln></p:spPr></p:pic>`
}
...
function slideXml(definition, index) {
  ...
  const images = definition.images.map((image, imageIndex) => imageShape(image, imageIndex + 5)).join('')
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld name="Slide ${index + 1}"><p:spTree>${groupProperties()}${title}${bullets}${images}${number}</p:spTree></p:cSld><p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr></p:sld>`
}
...
function contentTypes(slideCount) {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="png" ContentType="image/png"/><Default Extension="jpeg" ContentType="image/jpeg"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/ppt/presentation.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml"/><Override PartName="/ppt/slideMasters/slideMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideMaster+xml"/><Override PartName="/ppt/slideLayouts/slideLayout1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideLayout+xml"/><Override PartName="/ppt/theme/theme1.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/>${slides}</Types>`
}
...
function slideRels(images) {
  const media = images.map((image) => `<Relationship Id="${image.relationshipId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/${image.mediaName}"/>`).join('')
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>${media}</Relationships>`
}
...
function coreProperties(title) {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xml(title)}</dc:title><dc:creator>DeepSeek Harness Local Personal Assistant</dc:creator><cp:lastModifiedBy>DeepSeek Harness Local Personal Assistant</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${now}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${now}</dcterms:modified></cp:coreProperties>`
}

function appProperties(slideCount) {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>DeepSeek Harness Local Personal Assistant</Application><PresentationFormat>Widescreen</PresentationFormat><Slides>${slideCount}</Slides><Notes>0</Notes><HiddenSlides>0</HiddenSlides><MMClips>0</MMClips><ScaleCrop>false</ScaleCrop><Company>Local</Company><LinksUpToDate>false</LinksUpToDate><SharedDoc>false</SharedDoc><HyperlinksChanged>false</HyperlinksChanged><AppVersion>1.0</AppVersion></Properties>`
}

export function createTextOnlyPptx({ title, slides }) {
  const files = {
    '[Content_Types].xml': strToU8(contentTypes(slides.length)),
    '_rels/.rels': strToU8(rootRels),
    'docProps/core.xml': strToU8(coreProperties(title || slides[0].title)),
    'docProps/app.xml': strToU8(appProperties(slides.length)),
  ...
  slides.forEach((definition, index) => {
    ...
    const images = definition.images.map((image, imageIndex) => {
      mediaNumber += 1
      const mediaName = `image${mediaNumber}.${image.extension}`
      files[`ppt/media/${mediaName}`] = image.data
      return { ...image, relationshipId: `rId${imageIndex + 2}`, mediaName }
    })
    const slide = { ...definition, images }
    files[`ppt/slides/slide${number}.xml`] = strToU8(slideXml(slide, index))
    files[`ppt/slides/_rels/slide${number}.xml.rels`] = strToU8(slideRels(images))
  })
